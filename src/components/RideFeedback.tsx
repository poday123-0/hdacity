...
const RideFeedback = ({ tripId, driverName = "your driver", fare, onComplete }: RideFeedbackProps) => {
      ...
      <motion.div initial={{ y: 30 }} animate={{ y: 0 }} className="bg-card rounded-2xl shadow-2xl mx-4 w-full max-w-sm overflow-hidden">
        <div className="p-5 space-y-5">
            ...
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="space-y-3">
                ...
                {quickTags.map((tag) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all active:scale-95 ${
                      selectedTags.includes(tag) ? "bg-primary text-primary-foreground" : "bg-surface text-muted-foreground"
                    }`}
                  >
                    {tag}
                  </button>
              ...
              <div className="relative">
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-surface text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary resize-none"
                />
              </div>
            </motion.div>
          )}
          ...
          <button
            ...
            className="w-full bg-primary text-primary-foreground font-semibold py-3.5 rounded-xl text-base disabled:opacity-40 active:scale-[0.98] transition-transform"
          >
            {submitting ? "Submitting..." : rating === 0 ? "Select a rating" : "Submit feedback"}
          </button>

          <button onClick={onComplete} className="w-full text-sm text-muted-foreground font-medium py-1">
            Skip
          </button>
        </div>
      </motion.div>
