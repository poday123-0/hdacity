...
const TopBar = ({ onDriverMode, onRegisterDriver, onLogout, userName, userProfile, onNotificationPress, onProfileUpdate }: TopBarProps) => {
  ...
  // Place search debounce — local DB + Nominatim (filtered by service area)
  useEffect(() => {
    ...
    placeSearchDebounce.current = setTimeout(async () => {
      ...
      try {
        ...
        // Local DB search + service area polygons in parallel
        const [{ data: svcData }, { data: namedData }, areas] = await Promise.all([
        ...
        // Nominatim — filtered by service area polygons
        const nomP = fetch(
        ...
        const nomResults = (Array.isArray(nomData) ? nomData : [])
          .filter((r: any) => {
            ...
            return !existingNames.has((r.name || "").toLowerCase()) && isInsideAnyServiceArea(lat, lng, areas);
          })
          .map((r: any, i: number) => ({
            place_id: 1000 + i,
  ...
  return (
    <>
      <div className="absolute -top-6 left-0 right-0 z-[1100] capacitor-safe-top bg-gradient-to-b from-background/80 via-background/40 to-transparent">
        <div className="px-4 flex items-center justify-between py-[33px]">
          <button
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <button
                    ...
                    >
                      ...
                      <span className="text-[10px] font-semibold text-muted-foreground">Privacy</span>
                    </button>
                  {onLogout && (
                    <button
