...
const DriverMatching = ({ onCancel, driver }: DriverMatchingProps) => {
  ...
  const phone = driver?.phone || "+960 7771234";
  const avatarUrl = driver?.avatar_url;
  const bankAccounts = driver?.bank_accounts || [
    { id: "1", bank_name: "BML", account_number: "7730000012345", account_name: "Ibrahim Hassan", is_primary: true },
  ];
    ...
    >
      <div className="p-4 pb-6 space-y-4">
        ...
        <div className="bg-primary/10 rounded-xl p-4 text-center">
          <p className="text-xs text-primary font-semibold">Estimated arrival</p>
        ...
        <div className="flex gap-2">
          ...
          <button className="flex-1 flex items-center justify-center gap-2 bg-surface text-foreground rounded-xl py-3 font-semibold active:scale-[0.98] transition-transform">
            <MessageSquare className="w-4 h-4" />
            Message
          </button>
        </div>

        {/* Bank accounts - primary first */}
        {primaryBank && (
          <div className="space-y-2">
            ...
            <BankCard bank={primaryBank} copiedId={copiedId} onCopy={copyToClipboard} />
                ...
                <AnimatePresence>
                  {showAllBanks && otherBanks.map((bank) => (
                    <motion.div key={bank.id} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
                      <BankCard bank={bank} copiedId={copiedId} onCopy={copyToClipboard} />
                    </motion.div>
                  ))}
                </AnimatePresence>
              </>
