...
const Index = () => {
  ...
  const checkDriverProfile = useCallback(async (phone: string) => {
    ...
    // Check for pending/pending review/rejected driver profile
    if (data && (data.status === "Pending" || data.status === "Pending Review" || data.status === "Rejected")) {
  ...
  const handleConfirmRide = useCallback(async (selectedPaymentMethod?: "cash" | "transfer" | "wallet") => {
    ...
    try {
      ...
      if (bookingType === "scheduled") {
        ...
        // Vehicle-type match = currently active vehicle OR approved via driver_vehicle_types
        // (so multi-type center drivers like Car+Van receive both kinds).
        try {
          ...
          // SAME active vehicle type as the trip. Drivers approved for the
          // type but online as a different vehicle are excluded.
