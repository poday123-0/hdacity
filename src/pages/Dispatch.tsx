...
const dispatchTabs: { id: DispatchTab; label: string; icon: typeof LayoutDashboard }[] = [
  ...
  { id: "google_map", label: "HDA Map", icon: MapPin },
];
...
.in("status", ["requested", "accepted", "arrived", "started", "completed"])
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // Play accept chime when trip transitions to accepted
  ...
  // Realtime: play alert beep when driver reports a road closure (auto-approved)
  useEffect(() => {
    if (!isAuthed) return;
    const closureChannel = supabase
      ...
      .on(
        ...
        async (payload: any) => {
          ...
          if (row?.reported_by_type === "driver") {
            // Play a distinct double-beep alert (not SOS siren)
            try {
              const ctx = new (window.AudioContext || (window as any).webkitAudioContext)();
            ...
            if (row.reported_by) {
              const { data: profile } = await supabase
                ...
                .single();
              if (profile) reporterInfo = `${profile.first_name} ${profile.last_name}`;
            }
              ...
              return (
                <>
                  <p className="text-[10px] text-muted-foreground px-1">
                    ...
                    {" · "}
                    <span className="text-green-500">{filtered.filter((t: any) => t.status === "completed" && t.vehicle).length} completed</span>
                    {" · "}
                    <span className="text-orange-400">{filtered.filter((t: any) => t.status === "completed" && !t.vehicle).length} no vehicle</span>
                    {" · "}
